...
export async function POST(request: NextRequest) {
  try {
    ...
    if (userId) {
      AuditRepository.log({
        userId,
        action: "CREATE_MEDIATION_RULE",
        resource: "mediation_rules",
        resourceId: result.insertedId.toString(),
        details: { content: body.content?.substring(0, 50) } // Reduced detail for potentially long content
      });
    }
